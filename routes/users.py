...
def index():
    ...
    return render_template("usuarios.html", users=users, roles=ROLES, search=search, selected_role=role_filter)
...
def toggle(user_id):
    ...
    db.session.commit()
    flash(f"Usuario {'activado' if user.active else 'desactivado'}", "success")
    return redirect(url_for("users.index"))
